import { IProductDetail } from '../types/product_card';

export const productCardDetails: IProductDetail[] = [
  {
    title: 'Frontend Development',
    description:
      'Saya membuat aplikasi frontend yang fungsional, indah, dan responsif untuk web dengan teknologi modern.',
    svgIcon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        stroke="white"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        className="text-white">
        <polyline points="16 18 22 12 16 6"></polyline>
        <polyline points="8 6 2 12 8 18"></polyline>
      </svg>
    ),

    stack: true,
    bgColor: 'var(--chakra-colors-cyan-400)',
  },
  {
    title: 'Backend Development',
    description:
      'Saya membangun layanan backend berbasis cloud berkualitas tinggi agar aplikasi Anda dapat diskalakan dan menampung semua pengguna Anda.',
    svgIcon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        stroke="white"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        className="text-white">
        <path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z"></path>
      </svg>
    ),
    stack: true,
    bgColor: 'var(--chakra-colors-orange-400)',
  },
  // {
  //   title: 'UX Design',
  //   description:
  //     'I create applications for real user to match their needs and expectations, satisfy their needs and ease their lives.',
  //   svgIcon: (
  //     <svg
  //       xmlns='http://www.w3.org/2000/svg'
  //       width='24'
  //       height='24'
  //       viewBox='0 0 24 24'
  //       fill='none'
  //       stroke='white'
  //       strokeWidth='2'
  //       strokeLinecap='round'
  //       strokeLinejoin='round'
  //       className='text-white'
  //     >
  //       <polygon points='1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6'></polygon>
  //       <line x1='8' y1='2' x2='8' y2='18'></line>
  //       <line x1='16' y1='6' x2='16' y2='22'></line>
  //     </svg>
  //   ),

  //   stack: true,
  //   bgColor: 'var(--chakra-colors-purple-400)',
  // },
  // {
  //   title: 'UI Design',
  //   description:
  //     'I design beautiful user interfaces for applications to work on all screen sizes. Your colors, typography and more, all pixel perfect and beautiful.',
  //   svgIcon: (
  //     <svg
  //       xmlns='http://www.w3.org/2000/svg'
  //       width='24'
  //       height='24'
  //       viewBox='0 0 24 24'
  //       fill='none'
  //       stroke='white'
  //       strokeWidth='2'
  //       strokeLinecap='round'
  //       strokeLinejoin='round'
  //       className='text-white'
  //     >
  //       <path d='M12 19l7-7 3 3-7 7-3-3z'></path>
  //       <path d='M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z'></path>
  //       <path d='M2 2l7.586 7.586'></path>
  //       <circle cx='11' cy='11' r='2'></circle>
  //     </svg>
  //   ),

  //   stack: true,
  //   bgColor: 'var(--chakra-colors-teal-400)',
  // },
  {
    title: 'Closer involvement',
    description:
      'Saya melibatkan klien saya secara dekat dalam proses desain dan pengembangan untuk menghasilkan produk yang paling sesuai dengan harapan Anda.',
    svgIcon: (
      <svg
        stroke="white"
        fill="white"
        strokeWidth="0"
        viewBox="0 0 640 512"
        height="24"
        width="24"
        xmlns="http://www.w3.org/2000/svg">
        <path d="M434.7 64h-85.9c-8 0-15.7 3-21.6 8.4l-98.3 90c-.1.1-.2.3-.3.4-16.6 15.6-16.3 40.5-2.1 56 12.7 13.9 39.4 17.6 56.1 2.7.1-.1.3-.1.4-.2l79.9-73.2c6.5-5.9 16.7-5.5 22.6 1 6 6.5 5.5 16.6-1 22.6l-26.1 23.9L504 313.8c2.9 2.4 5.5 5 7.9 7.7V128l-54.6-54.6c-5.9-6-14.1-9.4-22.6-9.4zM544 128.2v223.9c0 17.7 14.3 32 32 32h64V128.2h-96zm48 223.9c-8.8 0-16-7.2-16-16s7.2-16 16-16 16 7.2 16 16-7.2 16-16 16zM0 384h64c17.7 0 32-14.3 32-32V128.2H0V384zm48-63.9c8.8 0 16 7.2 16 16s-7.2 16-16 16-16-7.2-16-16c0-8.9 7.2-16 16-16zm435.9 18.6L334.6 217.5l-30 27.5c-29.7 27.1-75.2 24.5-101.7-4.4-26.9-29.4-24.8-74.9 4.4-101.7L289.1 64h-83.8c-8.5 0-16.6 3.4-22.6 9.4L128 128v223.9h18.3l90.5 81.9c27.4 22.3 67.7 18.1 90-9.3l.2-.2 17.9 15.5c15.9 13 39.4 10.5 52.3-5.4l31.4-38.6 5.4 4.4c13.7 11.1 33.9 9.1 45-4.7l9.5-11.7c11.2-13.8 9.1-33.9-4.6-45.1z"></path>
      </svg>
    ),
    stack: false,
    bgColor: 'var(--chakra-colors-red-400)',
  },
  {
    title: 'Your product ships faster',
    description:
      'Tidak seperti perusahaan besar, saya dapat memberikan proses pengembangan produk yang lebih cepat dari awal hingga akhir untuk membantu peluncuran aplikasi Anda lebih cepat.',
    svgIcon: (
      <svg
        stroke="white"
        fill="white"
        strokeWidth="0"
        viewBox="0 0 512 512"
        height="24"
        width="24"
        xmlns="http://www.w3.org/2000/svg">
        <path d="M477.64 38.26a4.75 4.75 0 00-3.55-3.66c-58.57-14.32-193.9 36.71-267.22 110a317 317 0 00-35.63 42.1c-22.61-2-45.22-.33-64.49 8.07C52.38 218.7 36.55 281.14 32.14 308a9.64 9.64 0 0010.55 11.2l87.31-9.63a194.1 194.1 0 001.19 19.7 19.53 19.53 0 005.7 12L170.7 375a19.59 19.59 0 0012 5.7 193.53 193.53 0 0019.59 1.19l-9.58 87.2a9.65 9.65 0 0011.2 10.55c26.81-4.3 89.36-20.13 113.15-74.5 8.4-19.27 10.12-41.77 8.18-64.27a317.66 317.66 0 0042.21-35.64C441 232.05 491.74 99.74 477.64 38.26zM294.07 217.93a48 48 0 1167.86 0 47.95 47.95 0 01-67.86 0z"></path>
        <path d="M168.4 399.43c-5.48 5.49-14.27 7.63-24.85 9.46-23.77 4.05-44.76-16.49-40.49-40.52 1.63-9.11 6.45-21.88 9.45-24.88a4.37 4.37 0 00-3.65-7.45 60 60 0 00-35.13 17.12C50.22 376.69 48 464 48 464s87.36-2.22 110.87-25.75A59.69 59.69 0 00176 403.09c.37-4.18-4.72-6.67-7.6-3.66z"></path>
      </svg>
    ),
    stack: false,
    bgColor: 'var(--chakra-colors-blue-400)',
  },
];
